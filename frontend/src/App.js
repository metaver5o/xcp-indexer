...
const S = {
  ...
  navLink: a => ({ color:a?C.accent:C.muted, cursor:'pointer', fontSize:'0.82rem', fontWeight:a?600:400, padding:'6px 10px', borderRadius:'6px', background:a?`${C.accent}15`:'transparent' }),
  ...
  return (
    <div style={{ background:C.surface, borderBottom:`1px solid ${C.border}`, padding:'8px 24px', fontSize:'0.78rem' }}>
      <div style={{ maxWidth:'1280px', margin:'0 auto', display:'flex', alignItems:'center', gap:'16px', flexWrap:'wrap' }}>
        ...
        <span style={{ color:C.muted }}>Chain tip: <b style={{color:C.text}}>{fmt.num(tip)}</b></span>
        ...
        {!running && (
          <button style={{ ...S.btnSm, marginLeft:'auto' }} onClick={onSync}>↻ Sync now</button>
        )}
      </div>
...
function InscriptionBadge({ v }) {
  return v ? <span style={S.badge(C.green)}>inscription</span> : <span style={S.badge(C.muted)}>token</span>;
}
...
function Thumb({ tx, mime }) {
  ...
  if (cat === 'image') return (
    <img src={url} alt="" style={{ width:36, height:36, objectFit:'cover',
      borderRadius:6, display:'block', background:C.border }} />
  );
...
function Dashboard({ status, navigate }) {
  ...
  return (
    <div>
      <div style={S.grid4}>
        <div style={S.statCard}><div style={S.statVal}>{fmt.num(stats.issuanceCount)}</div><div style={S.statLbl}>Total Issuances</div></div>
        <div style={S.statCard}><div style={S.statVal}>{fmt.num(stats.assetCount)}</div><div style={S.statLbl}>Assets</div></div>
        ...
        <div style={S.statCard}><div style={S.statVal}>{fmt.num(stats.inscAssetCount)}</div><div style={S.statLbl}>Inscribed Assets</div></div>
      </div>
        ...
        <div style={S.card}>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:'8px' }}>
            {mimeStats.result.map(m => (
              <div key={m.mime_type} style={{ display:'flex', alignItems:'center', gap:'6px', cursor:'pointer' }}
                   onClick={() => navigate('issuances', { inscription:true, mime_type:m.mime_type })}>
      ...
      <div style={S.card}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
          ...
          <button style={S.btnSm} onClick={() => navigate('issuances', { inscription:true })}>View all →</button>
        </div>
        {!recentInsc ? <div style={{ color:C.muted, fontSize:'0.82rem' }}>Loading…</div> : (
          <table style={S.table}>
            <thead><tr>
              <th style={S.th}>Asset</th><th style={S.th}>From</th>
              <th style={S.th}>Block</th><th style={S.th}>MIME</th>
            </tr></thead>
            <tbody>
              {recentInsc.result?.map(iso => (
                <tr key={iso.tx_hash} style={{ cursor:'pointer' }} onClick={() => navigate('issuance', iso.tx_hash)}>
...
// ─── Issuances page ───────────────────────────────────────────────────────────
function IssuancesPage({ initialFilter={}, navigate }) {
  const [page,   setPage]   = useState(1);
  const [tab,    setTab]    = useState(initialFilter.inscription!=null ? (initialFilter.inscription?'inscriptions':'tokens') : 'all');
  const [mime,   setMime]   = useState(initialFilter.mime_type||'');
  const [search, setSearch] = useState('');
  const LIMIT = 50;

  const inscParam = tab==='inscriptions' ? '&inscription=true' : tab==='tokens' ? '&inscription=false' : '';
  const mimeParam = mime ? `&mime_type=${encodeURIComponent(mime)}` : '';
  const srcParam  = search ? `&source=${encodeURIComponent(search)}` : '';
  const url = `/issuances?limit=${LIMIT}&offset=${(page-1)*LIMIT}${inscParam}${mimeParam}${srcParam}`;
  const { data } = useApi(url, [page, tab, mime, search]);
        ...
        {!data ? <div style={{ color:C.muted }}>Loading…</div> : (
          ...
          <table style={S.table}>
            <thead><tr>
              <th style={S.th}></th>
              <th style={S.th}>Asset</th><th style={S.th}>From</th>
              <th style={S.th}>Block</th><th style={S.th}>MIME</th><th style={S.th}>Type</th>
            </tr></thead>
            <tbody>
              {data.result?.map(iso=>(
                <tr key={iso.tx_hash} style={{ cursor:'pointer' }} onClick={()=>navigate('issuance',iso.tx_hash)}>
                  ...
                  <td style={S.td}><span style={{...S.hash,color:C.accent2,maxWidth:180}} onClick={e=>{e.stopPropagation();navigate('asset',iso.asset)}}>{iso.asset}</span></td>
                  <td style={S.td}><span style={S.hash} onClick={e=>{e.stopPropagation();navigate('address',iso.source)}}>{fmt.addr(iso.source)}</span></td>
                  <td style={S.td}><span style={{color:C.muted}}>{fmt.num(iso.block_index)}</span></td>
                  <td style={S.td}><MimeBadge mime={iso.mime_type}/></td>
...
function AssetsPage({ navigate }) {
  ...
  const inscParam = tab==='inscriptions'?'&inscription=true': tab==='tokens'?'&inscription=false':'';
  const srchParam = search ? `&search=${encodeURIComponent(search)}` : '';
  const url = `/assets?limit=${LIMIT}&offset=${(page-1)*LIMIT}${inscParam}${srchParam}`;
  const { data } = useApi(url, [page, tab, search]);

  return (
    <div>
      <div style={S.h2}>Assets {data&&<span style={S.pill(C.muted)}>{fmt.num(data.total)}</span>}</div>
      <div style={{ display:'flex', gap:'8px', marginBottom:'14px', flexWrap:'wrap', alignItems:'center' }}>
        {['all','inscriptions','tokens'].map(t=>(
      ...
      <div style={S.card}>
        {!data ? <div style={{color:C.muted}}>Loading…</div> : (
          ...
          <table style={S.table}>
            <thead><tr>
              <th style={S.th}>Asset</th><th style={S.th}>Owner</th>
              <th style={S.th}>Supply</th><th style={S.th}>MIME</th>
              <th style={S.th}>Last Block</th><th style={S.th}>Type</th>
            </tr></thead>
            <tbody>
              {data.result?.map(a=>(
                <tr key={a.asset} style={{cursor:'pointer'}} onClick={()=>navigate('asset',a.asset)}>
                  <td style={S.td}><span style={{...S.hash,color:C.accent2,maxWidth:200}}>{a.asset}{a.asset_longname&&<span style={{color:C.muted,fontSize:'0.7rem'}}> ({a.asset_longname})</span>}</span></td>
                  <td style={S.td}><span style={S.hash} onClick={e=>{e.stopPropagation();navigate('address',a.owner)}}>{fmt.addr(a.owner)}</span></td>
...
function TextContent({ url }) {
  ...
  return (
    <pre style={{ fontFamily:C.mono, fontSize:'0.78rem', color:C.text,
      background:'#000', borderRadius:10, padding:16, overflow:'auto',
...
// ─── Issuance detail ──────────────────────────────────────────────────────────
function IssuanceDetail({ tx, navigate }) {
  ...
  return (
    <div>
      ...
      {/* Content viewer — front and center for inscriptions */}
      {iso.inscription===1 && (
        <div style={S.card}>
      ...
      <div style={S.card}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', fontSize:'0.83rem' }}>
          {[
            ['Asset',    <span style={{...S.hash,color:C.accent2,maxWidth:240}} onClick={()=>navigate('asset',iso.asset)}>{iso.asset}</span>],
...
// ─── Asset detail ─────────────────────────────────────────────────────────────
function AssetDetail({ asset, navigate }) {
  const { data } = useApi(`/assets/${encodeURIComponent(asset)}`);
  if (!data) return <div style={{color:C.muted}}>Loading…</div>;
  const { asset: a, history } = data.result;
  return (
    <div>
      <div style={S.h2}>{a.asset} {a.inscription?<span style={S.badge(C.green)}>inscription</span>:<span style={S.badge(C.muted)}>token</span>}</div>
      {a.asset_longname && <div style={{color:C.muted,marginBottom:16,fontSize:'0.85rem'}}>{a.asset_longname}</div>}
      <div style={S.card}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px', fontSize:'0.83rem' }}>
          {[
            ['Owner',       <span style={S.hash} onClick={()=>navigate('address',a.owner)}>{a.owner}</span>],
          ...
          {a.description && <div style={{gridColumn:'1/-1'}}><span style={{color:C.muted}}>Description: </span>
            <span style={{fontFamily:C.mono,fontSize:'0.72rem',wordBreak:'break-all'}}>{a.description.slice(0,200)}{a.description.length>200?'…':''}</span></div>}
        </div>
      </div>
      <div style={S.card}>
        <div style={S.sec}>Issuance History</div>
        <table style={S.table}>
          <thead><tr><th style={S.th}>Tx</th><th style={S.th}>Block</th><th style={S.th}>Qty</th><th style={S.th}>Status</th></tr></thead>
          <tbody>
            {history.map(h=>(
              <tr key={h.tx_hash} style={{cursor:'pointer'}} onClick={()=>navigate('issuance',h.tx_hash)}>
...
// ─── Address page ─────────────────────────────────────────────────────────────
function AddressPage({ addr, navigate }) {
    ...
    <div>
      ...
      {assets.length>0 && (
        <div style={S.card}>
          ...
          <table style={S.table}>
            <thead><tr><th style={S.th}>Asset</th><th style={S.th}>Supply</th><th style={S.th}>MIME</th><th style={S.th}>Type</th></tr></thead>
            <tbody>
              {assets.map(a=>(
                <tr key={a.asset} style={{cursor:'pointer'}} onClick={()=>navigate('asset',a.asset)}>
      ...
      {issuances.length>0 && (
        <div style={S.card}>
          <div style={S.sec}>Issuance History</div>
          <table style={S.table}>
            <thead><tr><th style={S.th}>Asset</th><th style={S.th}>Block</th><th style={S.th}>MIME</th><th style={S.th}>Type</th></tr></thead>
            <tbody>
              {issuances.map(iso=>(
                <tr key={iso.tx_hash} style={{cursor:'pointer'}} onClick={()=>navigate('issuance',iso.tx_hash)}>
...
function SearchBar({ navigate }) {
  ...
  const go = async () => {
    ...
    try {
      ...
      if (r.type==='block')     navigate('issuances', { blockIndex: r.blockIndex });
...
export default function App() {
  ...
  const navigate = (name, id, opts={}) => setPage({ name, id, ...opts });
  const triggerSync = () => fetch(`${API}/sync`, { method:'POST' }).then(()=>api('/status').then(setStatus));

  const render = () => {
    switch (page.name) {
      case 'dashboard':  return <Dashboard status={status} navigate={navigate}/>;
      case 'issuances':  return <IssuancesPage initialFilter={page.id||{}} navigate={navigate}/>;
      case 'assets':     return <AssetsPage navigate={navigate}/>;
      case 'issuance':   return <IssuanceDetail tx={page.id} navigate={navigate}/>;
      case 'asset':      return <AssetDetail asset={page.id} navigate={navigate}/>;
      case 'address':    return <AddressPage addr={page.id} navigate={navigate}/>;
  ...
  return (
    <div style={S.root}>
      <nav style={S.nav}>
        <span style={S.logo} onClick={()=>setPage({name:'dashboard'})}>⬡ XCP-Indexer</span>
        {NAV.map(n=>(
          <span key={n.key} style={S.navLink(page.name===n.key)} onClick={()=>setPage({name:n.key})}>{n.label}</span>
        ))}
